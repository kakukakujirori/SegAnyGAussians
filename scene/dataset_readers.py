...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder = None, masks_folder = None, mask_scale_folder = None, sample_rate = 1.0, allow_principle_point_shift = False):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        features = torch.load(os.path.join(features_folder, image_name.split('.')[0] + ".pt"), weights_only=True) if features_folder is not None else None
        masks = torch.load(os.path.join(masks_folder, image_name.split('.')[0] + ".pt"), weights_only=True) if masks_folder is not None else None
        mask_scales = torch.load(os.path.join(mask_scale_folder, image_name.split('.')[0] + ".pt"), weights_only=True) if mask_scale_folder is not None else None
